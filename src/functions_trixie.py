...
async def videoProcessing(identifier, imshow=False):
    ...
    while True:
        ...
        if len(face_locations) > 0:
            ...
            face_landmarks_list = fr.face_landmarks(scaled_rgb, [face_locations[0]])
            if face_landmarks_list:
                ...
                if left_eye and right_eye:
                    # Visualize the eye landmarks (yellow dots)
                    ...
                    # Display live EAR value on screen
                    cv2.putText(scaled_bgr, f"Live EAR (Threshold 0.20): {ear:.2f}", (10, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)

                    if ear < EYE_AR_THRESH:
                        blink_counter += 1
                        cv2.putText(scaled_bgr, "BLINKING...", (10, 60),
            ...
            try:
                ...
                if person is None:
                    # New user: save full-resolution face crop
                    frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
                    face_img = frame_bgr[(top * 2):(bottom * 2), (left * 2):(right * 2)]
                    identifier.addNew(face_img, face_encoding)
                    accessDenied(name="New User", reason="Not Enrolled")
                    cv2.putText(scaled_bgr, "NEW USER CAPTURED", (left, top - 10),
                    ...
                    if liveness_passed:
                        ...
                        cv2.putText(scaled_bgr, "LIVENESS PASSED: UNLOCKED", (left, top - 10),
                        ...
                        cv2.putText(scaled_bgr, "AUTHORIZED: PLEASE BLINK", (left, top - 10),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                else:
                    accessDenied(name=person_name, reason="Policy Denied")
                    cv2.putText(scaled_bgr, "ACCESS DENIED", (left, top - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    liveness_passed = False
        ...
        # Encode BGR frame as JPEG and stream to web dashboard
        ret_enc, v = cv2.imencode('.jpg', scaled_bgr)
        if ret_enc:
            identifier.setView(v.tobytes())
